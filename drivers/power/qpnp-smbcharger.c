...
static int smbchg_debug_mask = PR_STATUS | PR_INTERRUPT;
module_param_named(
...
static int get_prop_batt_capacity(struct smbchg_chip *chip)
{
	...
	if (chip->report_soc == -EINVAL)
		chip->report_soc = capacity;

	if (chip->report_soc != capacity && capacity > 10) {
		capacity = (capacity + chip->report_soc) / 2 + 1;
...
#define DEFAULT_BATT_CAPACITY_FULL_DESIGN      3000000
static int get_prop_batt_capacity_full_design(struct smbchg_chip *chip)
{
        int uah, rc;
        ...
        if (rc) {
                ...
                uah = DEFAULT_BATT_CAPACITY_FULL_DESIGN;
        }
        return uah;
}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = get_prop_batt_capacity_full_design(chip);
		break;
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	/* set initial usb current to 500ma */
	smbchg_set_high_usb_chg_current(chip, 0);
	check_battery_type(chip);
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	schedule_delayed_work(&chip->reg_work, 0);
